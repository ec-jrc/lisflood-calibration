...
# inflow.tss is created using CAL_4_PERFORM_CAL.py
...
def RunModel(Individual):
	...
	template_xml_new = template_xml_new.replace('%StepEnd',str(Cal_End_Step)) # Time step of forcing at which to end simulation
	...
	p = Popen(RunLISFLOOD_template[:-4]+run_rand_id+'.bat', stdout=PIPE, stderr=PIPE, bufsize=16*1024*1024)
	...
	correlation = HydroStats.correlation(s=Qsim,o=Qobs,warmup=WarmupDays)
	NSlog = HydroStats.NSlog(s=Qsim,o=Qobs,warmup=WarmupDays)
	apb2 = HydroStats.apb2(s=Qsim,o=Qobs,warmup=WarmupDays)

	print "   run_rand_id: "+str(run_rand_id)+", R: "+"{0:.3f}".format(correlation)+", NSlog: "+"{0:.3f}".format(NSlog)+", B: "+"{0:.1f}".format(apb2)

	with open(os.path.join(path_subcatch,"runs_log.csv"), "a") as myfile:
		myfile.write(str(run_rand_id)+","+str(correlation)+","+str(NSlog)+","+str(apb2)+"\n")

	return correlation, NSlog, apb2 # If using just one objective function, put a comma at the end!!!
...
if __name__ == "__main__":
	...
	effmax = np.zeros(shape=(ngen+1,3))*np.NaN
	effmin = np.zeros(shape=(ngen+1,3))*np.NaN
	effavg = np.zeros(shape=(ngen+1,3))*np.NaN
	effstd = np.zeros(shape=(ngen+1,3))*np.NaN
	for ii in range(3):
	...
	gen = 0
	print ">> gen: "+str(gen)+", effmax_R: "+"{0:.3f}".format(effmax[gen,0])+", effmax_NSlog: "+"{0:.3f}".format(effmax[gen,1])+", effmin_B: "+"{0:.1f}".format(effmin[gen,2])
	...
	front_history = pandas.DataFrame({'gen':range(gen),
									  'effmax_R':effmax[:,0],'effmax_NSlog':effmax[:,1],'effmax_B':effmax[:,2],
									  'effmin_R':effmin[:,0],'effmin_NSlog':effmin[:,1],'effmin_B':effmin[:,2],
									  'effstd_R':effstd[:,0],'effstd_NSlog':effstd[:,1],'effstd_B':effstd[:,2],
									  'effavg_R':effavg[:,0],'effavg_NSlog':effavg[:,1],'effavg_B':effavg[:,2]})
	...
	effover = 1 - np.sqrt((1-front[:,0]) ** 2 + (1-front[:,1]) ** 2 + (front[:,2]/100) ** 2)
	...
	pareto_front = pandas.DataFrame({'effover':effover[ind],'R':front[ind,0],'NSlog':front[ind,1],'B':front[ind,2]})
	for ii in range(len(ParamRanges)):
	...
	for ii in range(0,len(ParamRanges)):
		template_xml_new = template_xml_new.replace("%"+ParamRanges.index[ii],str(Parameters[ii]))
	template_xml_new = template_xml_new.replace('%ForcingStart',ForcingStart.date().strftime('%d/%m/%Y')) # Date of forcing start
	template_xml_new = template_xml_new.replace('%StepStart',str(1)) # Time step of forcing at which to start simulation
	template_xml_new = template_xml_new.replace('%StepEnd',str(Forcing_End_Step)) # Time step of forcing at which to end simulation
	template_xml_new = template_xml_new.replace('%CatchmentDataPath',CatchmentDataPath) # Directory with forcing for the whole region
