...
def write_ranked_solution(path_subcatch):
...
def write_pareto_front(cfg, path_subcatch, pHistory):
...
def run_calibration(cfg, obsid, path_subcatch, station_data, model, lock_mgr):
    ...
    if os.path.isfile(history_file) and os.path.getsize(history_file) > 0:
        population = restore_calibration(deap_param, lock_mgr, toolbox, halloffame, history_file, effmax, effmin, effavg, effstd, conditions)
    else:
    ...
    pHistory = write_ranked_solution(path_subcatch)

    write_pareto_front(cfg, path_subcatch, pHistory)
