...
inflow_tss_cal = os.path.join(path_subcatch,"inflow","inflow_cal.tss")
inflowflag = str(0)
...
Cal_Start=datetime.strptime(row['Cal_Start'],'%d/%m/%Y %H:%M').strftime('%Y-%m-%d')
#al_End = str(datetime.datetime.strptime(row['Cal_End'],"%d/%m/%Y %H:%M"))
Cal_End=datetime.strptime(row['Cal_End'],'%d/%m/%Y %H:%M').strftime('%Y-%m-%d')
...
def RunModel(Individual):
    ...
    st = os.stat(path_subcatch+'/runLF_linux'+run_rand_id+'.bat')
    os.chmod(path_subcatch+'/runLF_linux'+run_rand_id+'.bat', st.st_mode | stat.S_IEXEC)
    print path_subcatch+'/runLF_linux'+run_rand_id+'.bat'
    p = Popen(path_subcatch+'/runLF_linux'+run_rand_id+'.bat', stdout=PIPE, stderr=PIPE, bufsize=16*1024*1024)
...
if __name__ == "__main__":
    ...
    # Terminate the optimization after ngen generations
            ...
            if gen >= 6:
                if (effmax[gen,0]-effmax[gen-1,0]) < 0.001:
    ...
    #print 'forc',ForcingStart
    template_xml_new = template_xml_new.replace('%CalStart', ForcingStart.date().strftime('%d/%m/%Y %H:%M')) # Time step of forcing at which to start simulation
    template_xml_new = template_xml_new.replace('%CalEnd', ForcingEnd.date().strftime('%d/%m/%Y %H:%M')) # Time step of forcing at which to end simulation
    template_xml_new = template_xml_new.replace('%CatchmentDataPath',CatchmentDataPath) # Directory with forcing for the whole region
