...
def main(stations_file, ldd_map, path_result, path_temp):
	"""Compute hydrological dependencies between stations.

	Reads station data, creates catchment masks, computes interstation regions,
	sampling frequency, and station links with inflow locations.

	Parameters
	----------
	stations_file : str
		Path to stations data CSV file (with LisfloodX, LisfloodY columns, station ID as index).
	ldd_map : str
		Path to the LDD (local drain direction) map.
	path_result : str
		Output directory for stations results (gauges.map, inlets.map, stations_links.csv, etc.).
	path_temp : str
	...
	------
	Exception
		If any station ID >= 999999 or if station location conflicts are detected.
	"""
	...
	pcrasterCommand(pcrcalc + " 'F0 = scalar(F1)*0-1'", {"F0": interstation_regions_map, "F1": ldd_map})
	...
	pcrasterCommand(pcrcalc + " 'F0 = scalar(F1)*0'", {"F0": sampling_frequency_map, "F1": ldd_map})
	for index, row in stationdata.iterrows():
		sys.stdout.write(".")
		catchment_map = os.path.join(path_temp, "catchmask%05d.map" % float(index))
	...
	for index, row in stationdata.iterrows():
		...
		# find subcatchments inside this catchment
		catchment_map = os.path.join(path_temp, "catchmask%05d.map" % float(index))
		...
		for line in f3.readlines():
			...
			if int(row['SamplingFrequency'] + 1) == int(stationdata.loc[subcatchment, 'SamplingFrequency']):
				...
				pcrasterCommand(pcrcalc + " 'F0 = cover(F1,0) eq "+str(subcatchment)+"'", {"F0": tmp_map, "F1": station_map})
				pcrasterCommand(pcrcalc + " 'F0 = upstream(F1,scalar(F2))*"+str(subcatchment)+"'", {"F0": tmp2_map, "F1": ldd_map, "F2": tmp_map})

				# check if inlet overlaps with another inlet, if yes then move 1 more pixel downstream
				value = str(2)
				while float(value) > 1:
					pcrasterCommand(pcrcalc + " 'F0 = scalar(F1!=0)+scalar(F2!=0)'", {"F0": tmp3_map, "F1": inlets_map, "F2": tmp2_map})
					pcrasterCommand(pcrcalc + " 'F0 = if(F0!=0 then F0)'", {"F0": tmp3_map})
					pcrasterCommand(map2col + " F0 F1", {"F0": tmp3_map, "F1": tmp_txt})
					with open(tmp_txt, "r") as f:
						for line in f.readlines():
							(X, Y, value) = line.split()
							if float(value) > 1: # this means there is overlap, therefore move inlet further downstream
								pcrasterCommand(pcrcalc + " 'F0 = upstream(F1,scalar(F2!=0))*"+str(subcatchment)+"'", {"F0": tmp3_map, "F1": ldd_map, "F2": tmp2_map})
								pcrasterCommand(pcrcalc + " 'F0 = F1'", {"F0": tmp2_map, "F1": tmp3_map})

				pcrasterCommand(pcrcalc + " 'F0 = F0+F1'", {"F0": inlets_map, "F1": tmp2_map})
				pcrasterCommand(pcrcalc + " 'F0 = if(F0!=0 then F0)'", {"F0": tmp2_map})
				pcrasterCommand(map2col + " F0 F1", {"F0": tmp2_map, "F1": tmp_txt})
				with open(tmp_txt, "r") as f:
					for line in f.readlines():
						(X, Y, value) = line.split()
...
if __name__ == "__main__":
	...
	main(args.stations_data, args.ldd, args.stations_dir, args.tmp_dir)
