...
class Projection():
    """Conversion object local projection and WGS84 lon/lat"""

    def __init__(self, name, proj_string):

        self.name = name
        self.proj_string = proj_string

        # coord system
        self.ll_proj = pyproj.Proj('epsg:4326')
        self.xy_proj = pyproj.Proj(proj_string)


class Projection_LAEA(Projection):
    """Conversion object EPSG3035 (Lambert azimuthal equal area) to EPSG:4326 lon/lat"""
    def __init__(self, lat_0, lon_0, x_0, y_0, ellps, proj_keys):
        ...
        proj_string = '+proj=laea +lat_0={} +lon_0={} +x_0={} +y_0={} +ellps={} {}'.format(lat_0, lon_0, x_0, y_0, ellps, proj_keys)

        super().__init__('lambert_azimuthal_equal_area', proj_string)
...
class Domain_LAEA(Domain):
    def __init__(self, name, projection, dx, dy, xul, yul, nx, ny):
        super().__init__(name, projection)

        self.dx = dx
        self.dy = dy
        self.nx = nx  # used to be ncol
        self.ny = ny  # used to be nrow
        self.xul = xul
        self.yul = yul
        self.xbr = xul+self.dx*self.nx
        self.ybr = yul+self.dy*self.ny

        # get upper left coordinates at the centre of the cell
        self.xulc = self.xul+self.dx/2.
        self.yulc = self.yul+self.dy/2.
        # get bottom right coordinates at the centre of the cell
        self.xbrc = self.xulc+self.dx*(self.nx-1)
        self.ybrc = self.yulc+self.dy*(self.ny-1)
...
with dask.config.set(scheduler='threads'):
    ...
    for index, row in stationdata_sorted.iterrows():
        ...
        if len(Series[Series==index]) == 0: # Only process catchments whose ObsID is in the CatchmentsToProcess.txt file
            continue
        print("=================== "+str(index)+" ====================")
        print(">> Starting map subsetting for catchment "+str(index)+", size "+str(row['CatchmentArea'])+" pixels...")
                ...
                if os.path.isfile(fileout) and os.path.getsize(fileout) > 0:
                    ...
                    if ext == ".map":
                        ...
                        # DD New fix with WKT2 standard, but isnt backward compatible for a single map genua3.nc
                          ...
                          mapAttrClipDom = getMapAttributes(5, 'EPSG:3035', 'XDOM', rasterOrigin=origin, nxny=nxny)

                          # Define domain object
                          if mapAttrClipDom['projection'] == 'EPSG:3035':
                            projection = Projection_LAEA('lambert_azimuthal_equal_area',
                                                             lat_0=52,
                                                             lon_0=10,
                                                             x_0=4321000,
                                                             y_0=3210000,
                          ...
                          domain = Domain_LAEA('EFAS Extended Domain 5km',
                                                   projection,
